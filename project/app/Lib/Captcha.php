...
class Captcha{
    ...
    */
    public static function reCaptcha(){
        $reCaptcha = Extension::where('act', 'google-recaptcha2')->where('status', 0)->first();
    ...
    */
    public static function customCaptcha($width = '100%', $height = 46, $bgColor = '#003'){
        ...
        $textColor = '#'. SETTING['base_color'];
        $captcha = Extension::where('act', 'custom-captcha')->where('status', 0)->first();
        if (!$captcha) {
            return 0;
        }
    ...
    */
    public static function verifyGoogleCaptcha(){
        $pass = true;
        $googleCaptcha = Extension::where('act', 'google-recaptcha2')->where('status', 0)->first();
        if ($googleCaptcha) {
    ...
    */
    public static function verifyCustomCaptcha(){
        $pass = true;
        $customCaptcha = Extension::where('act', 'custom-captcha')->where('status', 0)->first();
        if ($customCaptcha) {
