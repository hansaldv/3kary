...
class NotifyProcess {
    ...
    */
	protected function getMessage(){
		...
		if ($user && $template) {
		    $message = $this->replaceShortCode($user->fullname,$user->username,SETTING[$globalTemplate],$template->$body);
		    if (empty($message)) {
		        $message = $template->$body;
		    }
		}else{
			$message = $this->replaceShortCode($this->receiverName,$this->toAddress,SETTING[$globalTemplate],$this->message);
		}
